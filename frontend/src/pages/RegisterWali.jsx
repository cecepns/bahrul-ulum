...
const RegisterWali = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.username || !formData.email || !formData.password || !formData.nis_santri) {
    ...
    setIsLoading(true);
    try {
      const res = await request.post(API_ENDPOINTS.AUTH.REGISTER_WALI, formData);
      if (res.success) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-12 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white rounded-2xl border border-slate-100 shadow-xl p-8 z-10">
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <h2 className="text-2xl font-bold text-slate-800">Registrasi Wali Santri</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">Nomor Induk Santri (NIS) Anak</label>
            <div className="relative">
              <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400">
                <CreditCard size={16} />
              </span>
              <input
                type="text"
                name="nis_santri"
                value={formData.nis_santri}
                onChange={handleChange}
                className="w-full pl-10 pr-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all font-medium text-slate-750 placeholder:text-slate-400 text-sm"
                placeholder="Masukkan NIS Santri yang aktif"
              />
            </div>
          </div>
